...
module.exports = (options, req, res, next) => {
  ...
  busboy.on('file', (fieldname, file, name, encoding, mime) => {
    ...
    file.on('limit', () => {
      debugLog(options, `File size limit reached for ${fieldname} -> ${name}, bytes: ${getFileSize()}`);
      ...
      if (options.abortOnLimit) {
        debugLog(options, `Aborting upload because of size limit ${fieldname} -> ${name}, bytes: ${getFileSize()}`);
    ...
    debugLog(options, `New file opload ${fieldname} -> ${name}, bytes: ${getFileSize()}`);
  });
